...
namespace COD.Platform.Messaging.AzureServiceBus
{
	public class AzureSubscriptionClient<TContent> : SubscriptionClient<TContent>
	{
		...
		async Task HandleMessage(Message message, CancellationToken token)
		{
			...
			try
			{
				await Task.Run(() => OnMessageReceived(this, new MessagedReceivedArguments<TContent> { Message = AzureInboundMessage<TContent>.LoadFromBrokeredMessage(bm, this.subscriptionType), ReceivedTimestamp = DateTimeOffset.UtcNow }));
			}
			catch (Exception ex)
			{
		...
		public override void Unsubscribe()
		{
			if (!asbSubscription.IsClosedOrClosing)
			{
				...
				if (!durable)
					asbService.DeleteSubscription(asbSubscription.Name, asbSubscription.TopicPath);
			}
		}
